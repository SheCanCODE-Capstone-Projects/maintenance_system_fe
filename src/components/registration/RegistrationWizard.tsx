...
function Select({ label, name, value, choose, disabled = false }: { label: string; name: string; value: string; choose: (name: string, value: string) => void; disabled?: boolean }) { return <label className="reg-field"><span>{label} *</span><select value={value} onChange={e => choose(name, e.target.value)} required disabled={disabled}><option value="">{disabled ? "Select district first" : `Select ${label.toLowerCase()}`}</option>{(name === "district" ? ["Gasabo", "Kicukiro", "Nyarugenge", "Musanze", "Muhanga"] : ["Kacyiru", "Remera", "Cyeza", "Nyamabuye"]).map(item => <option key={item}>{item}</option>)}</select></label>; }
